        ...
        for event in stdin().events() {
            match event {
                ...
                Ok(ev) => match ev {
                    ...
                    Event::Key(Key::Char('\n')) => self.backend.newline(&mut self.cursor),
                    Event::Key(Key::Backspace) => self.backend.backspace(&mut self.cursor),
                    Event::Key(Key::Char(c)) => self.backend.insert(c, &mut self.cursor),
                    _ => {},
                },
                Err(e) => panic!("Error: {}", e),
            }
